...
| 3 | The Lagrangian bound is tighter than the LP bound **where price is a multiple of GPU count** — and it is *not established* where it is not | **Medium** (was High; **downgraded by F34**) | Uniform +5.20 pp [4.40, 5.98] on 25/25 and structured +13.44 pp [10.28, 16.70] on 22/22 — but heterogeneous **−2.17 pp [−6.99, 2.13], tighter on only 10/17** | **Already found.** F34. Note theory says the dual optimum *must* be ≥ the LP bound, so the shortfall is our subgradient failing to reach it (O5), not the relaxation |
...
| "Track B's bound is **strictly tighter on 30 of 30**" | F7, F12, F30 | **True only where price is a multiple of GPU count.** On the heterogeneous generator it is tighter on 10 of 17, paired −2.17 pp [−6.99, 2.13] — not established. Quote it with the pricing regime attached (F34) |
